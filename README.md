...
![메인 기능 시연](./assets/메인기능.gif)

> 📺 고화질 영상은 [`assets/메인기능.mp4`](./assets/메인기능.mp4) (2.2MB).
...
### 프로젝트 / 세션 목록
- **프로젝트 목록**: 카드 그리드 + **검색 / 정렬(Recent·Name) / 페이지네이션**(9개/페이지)
- **프로젝트 숨김** — 카드 hover 시 `×` 버튼(localStorage `projectList.hidden.v1`), 툴바의 `Hidden (N)` 버튼으로 숨김 목록 뷰 진입, `↺` 버튼으로 복구
- **세션 숨김** — 프로젝트 뷰 좌측 세션 사이드바의 각 행 hover 시 `×` 버튼(localStorage `sessionSidebar.hidden.v1`), 사이드바 헤더의 `Hidden (N)` 버튼으로 숨김 뷰 진입
  - 현재 열려있는 세션은 숨기더라도 사이드바에서 즉시 사라지지 않음 (뷰 깜빡임 방지)
- 숨긴 프로젝트 / 세션의 메모는 `/memos` 탭과 `/editor` picker에서도 자동 제외 (공유 헬퍼 `src/state/hiddenProjects.js` · `hiddenSessions.js`)
...
- **Subagent task notification** — Claude Code가 주입하는 `<task-notification>` (background task/subagent 완료 알림)을 유저 버블이 아닌 Claude의 접기 영역(thinking 블록)으로 표시. 빈 유저 버블 방지
- **IDE 메타 wrapper 제거** — `<ide_selection>` / `<ide_opened_file>` 처럼 IDE가 프롬프트에 주입하는 컨텍스트 블록은 유저 버블에서 숨김 (tool_result 덤프가 사용자 텍스트로 오염되는 것 방지)
- **슬래시 커맨드** — 유저 버블 안 인라인 코드 블록으로 표시 (`` `/commit` `` 형태)
...
- **Download HTML** — 서버 파이프라인(`POST /api/editor/export`): shiki 하이라이팅 + `formatTools` + pre-rendered 정적 HTML(JS 불필요). 공유 렌더러(`renderDocBody`)가 프로젝트 대화 뷰와 동일한 클래스명·구조로 출력 → 긴 유저 버블은 인라인 스크립트로 "더 보기" collapsible 자동 적용
- **Copy MD** — 각 블록의 원본 대화 턴(`### 👤 You` / `### 🤖 Claude` + `> body`) 포함 (공유 `renderTurns`), 메모 사이드 패널 preview의 markdown export와 동일 포맷. 슬래시 커맨드는 heading이 아닌 본문 첫 줄(`> /commit`)로 삽입 — 원문 그대로 복구 가능
- Preview 모드 토글

> **아키텍처: 공유 렌더러로 4 surface 통일**
>
> 대화 메시지 HTML 렌더링은 **`src/lib/renderMessageHtml.js`** 하나가 담당. 이 모듈이 생산하는 HTML을:
> - **React 앱** (프로젝트 대화 뷰, Editor, SessionMemoEditPage) → `dangerouslySetInnerHTML`로 사용
> - **Download HTML** → 서버가 `renderDocBody(payload)` 호출 → pre-rendered 정적 HTML (JS 불필요)
> - **viewer.js** (/preview) → esbuild가 shared import를 번들 → `renderNodes()` 호출
>
> 문서 레이아웃(h1, lede, meta, memo sections)은 **`MemoDoc.jsx`** + **`EditorMemoBlock.jsx`**가 React 앱 두 surface에 공유.
> Markdown export는 **`memoMarkdown.js`**(`renderMemoSection` · `renderTurns`)를 서버/클라이언트 공유.
>
> **슬래시 커맨드 표시 방식 같은 UI 변경 → `renderMessageHtml.js` 1곳만 수정 → 4 surface 모두 반영.**
...
- **연결된 서버 목록** (runtime): `./data/servers.json` *(gitignored — SSH 호스트 정보 포함)*
- **빌드 산출물**: `./dist/` (`viewer.min.js` — /preview 전용; Download HTML은 pre-rendered 정적 HTML로 viewer.js 불포함)
...
| Method | Path | 설명 |
|---|---|---|
| `GET` | `/api/sessions/:sessionId/memos/export` | 세션 메모 다운로드용 self-contained HTML |
| `GET` | `/api/sessions/:sessionId/memos/preview` | 라이브 HTML (edit 가능) — 레거시. 메인 편집은 in-app `#/sessions/:pid/:sid/edit` 라우트로 통합됨 |
| `GET` | `/api/sessions/:sessionId/memos/markdown` | 세션 메모 Markdown |
| `POST` | `/api/editor/export` | `/editor`의 cross-session doc을 pre-rendered HTML로 다운로드 (body: `{docTitle, intro, blocks[]}`). 위의 `/memos/export`와 동일 파이프라인(shiki + formatTools + 공유 렌더러) |
...
| Method | Path | 설명 |
|---|---|---|
| `GET` | `/api/ssh-hosts` | `~/.ssh/config` 호스트 목록 |
| `GET` | `/api/servers` | 연결된 서버 목록 |
| `POST` | `/api/servers` | 서버 추가 (`{sshAlias}`) |
| `DELETE` | `/api/servers/:id` | 서버 삭제 |
| `GET` | `/api/servers/:id/projects` | 원격 프로젝트 목록 |
| `GET` | `/api/servers/:id/projects/:pid/sessions` | 원격 세션 목록 |
| `GET` | `/api/servers/:id/projects/:pid/sessions/:sid` | 원격 세션 내용 |
...
2. **Shift+클릭**으로 범위 선택, 헤더의 **`select all`** 체크박스로 전체 선택
...
src/lib/                      # 서버/클라이언트 공용 ESM
  ...
  editorExport.js             # /editor의 Markdown 빌더 + download blob 헬퍼 (HTML은 서버 위임)
  formatTools.js              # tool_use 블록 포맷 (공유)
  parseMessages.js, ...
src/state/                    # 클라이언트 스토어
  ...
  hiddenSessions.js           # sessionSidebar.hidden.v1 read/write (공유 헬퍼)
src/components/
...
src/pages/
  ...
  ...
export-template/              # HTML 템플릿 (CSS) + viewer.js (/preview edit-mode 전용, renderMessageHtml.js import)

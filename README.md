...
- **숨김 / 숨김 관리** — 카드 hover 시 `×` 버튼으로 숨김(localStorage), 툴바의 `Hidden (N)` 버튼으로 숨김 목록 뷰 진입, `↺` 버튼으로 복구
...
- 동일한 toolbar UX(드래그 / ↑↓ / ✕)
- localStorage(`editor.draft.v1`) 자동 저장, Banner의 `✕` 아이콘으로 초기화
- Download HTML / Copy MD / Preview 모드 토글

> Editor와 세션 메모 편집은 공유 React 컴포넌트(`MemoDoc.jsx` + `EditorMemoBlock.jsx`)를 사용해 항상 같은 룩&필 유지. 한 곳을 고치면 양쪽이 같이 반영됨.
...
src/lib/                      # 서버/클라이언트 공용 ESM (parseMessages, formatTools, editorExport, ...)
src/state/                    # 클라이언트 스토어

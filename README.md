...
- **Download HTML** — 메모 사이드 패널의 Download와 **동일 서버 파이프라인** (`POST /api/editor/export`): shiki 코드 하이라이팅 + `formatTools` + `viewer.js` 번들 + template.html. 각 블록의 `(sourceProjectId, sourceSessionId, messageUuids)`로 서버가 세션 메시지 로드해서 셀프-컨테인드 HTML 생성. 단일 세션 메타 대신 `docTitle + intro`가 들어감
- **Copy MD** — 각 블록의 원본 대화 턴(`### 👤 You` / `### 🤖 Claude` + `> body`) 포함 (공유 `renderTurns`), 메모 사이드 패널 preview의 markdown export와 동일 포맷
- Preview 모드 토글

> Editor와 세션 메모 편집은 공유 React 컴포넌트(`MemoDoc.jsx` + `EditorMemoBlock.jsx`)를 사용. Export도 전 surface가 같은 파이프라인 공유:
> - **HTML export**: 서버 `buildMemoExport`(세션) / `buildCrossSessionExport`(editor) 둘 다 `prepareNode` (shiki + formatTools) + `template.html` + `viewer.js` 사용
> - **Markdown export**: 서버 `renderMemosMarkdown` / 클라이언트 `/editor` Copy MD 모두 공유 lib(`src/lib/memoMarkdown.js`의 `renderMemoSection` · `renderTurns`) 사용
>
> 한 곳 고치면 모든 surface에 반영됨.
...
| `POST` | `/api/editor/export` | `/editor`의 cross-session doc을 self-contained HTML로 다운로드 (body: `{docTitle, intro, blocks[]}`). 위의 `/memos/export`와 동일 파이프라인(shiki + formatTools + viewer.js) 재사용 |
